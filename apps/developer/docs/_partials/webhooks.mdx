...
body, _ := json.Marshal(map[string]any{
  "url":    "https://your-app.com/webhooks/monerium",
  "secret": secret,
  "types":  []string{"profile.updated", "profile.error", "iban.updated", "order.created", "order.updated"},
})
...
| `profile.error`   | KYC profile data was rejected as invalid and skipped during ingestion |
| `iban.updated`    | IBAN details or state changed    |
| `order.created`   | New order placed                 |
| `order.updated`   | Order state changed              |

`profile.error` is **opt-in** — add it to the `types` array when creating or updating a subscription. It is not included by default. The payload lists the `field`(s) that were rejected and the `reason`; the profile stays `incomplete` until the invalid data is corrected and resubmitted.
