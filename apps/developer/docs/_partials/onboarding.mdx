...
```go
body, _ := json.Marshal(map[string]string{"kind": "personal"}) // or "corporate"

req, _ := http.NewRequest("POST", "https://api.monerium.dev/profiles", bytes.NewBuffer(body))
...
// Store profile.ID — required for all subsequent requests
```
