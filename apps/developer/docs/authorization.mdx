...
## Authorization code flow with proof key for code exchange (PKCE) {#pkce}
...
Endpoint documentation: [Authorization](pathname:///api#tag/auth/operation/auth)
...
Endpoint documentation: [Access Token](pathname:///api#tag/auth/operation/auth-token)
